trigger SalesforceProjectTrigger on Salesforce_Project__c (before insert, after insert, before update, after update) {
    ...
    if (Trigger.isAfter && Trigger.isInsert) {
        ...
        SPTTriggerHandler.updateDescriptionFuture(Trigger.newMap.keySet());
        system.debug('JUST CALLED future method');

        //call handler method to insert default salesforce ticket.
        SPTTriggerHandler.createDefaultTicket(Trigger.New);
    ...
    if (Trigger.isAfter && Trigger.isUpdate) {
        SPTTriggerHandler.spStatusCompleted(Trigger.New, Trigger.Old, Trigger.NewMap, Trigger.OldMap);
    }
}
